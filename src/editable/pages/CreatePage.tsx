...
const fieldClass = 'rounded-2xl border border-[rgba(26,50,99,0.12)] bg-white px-4 py-3 text-sm font-bold text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-muted-text)]/45 focus:border-[var(--slot4-accent)]'
...
export default function CreatePage() {
  ...
  if (!session) {
    return (
      <EditableSiteShell>
        <main className="min-h-screen bg-[var(--slot4-cream)] px-4 py-16 text-[var(--slot4-page-text)] sm:px-6 lg:px-8">
          <section className="mx-auto grid max-w-5xl gap-8 rounded-[2.8rem] border border-[rgba(26,50,99,0.12)] bg-white p-7 shadow-[0_30px_90px_rgba(26,50,99,0.08)] md:grid-cols-[0.9fr_1.1fr] md:p-10">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login" className="inline-flex items-center gap-2 rounded-[1rem] bg-[var(--slot4-accent)] px-6 py-3 text-sm font-black text-[var(--slot4-dark-bg)]">Login <ArrowRight className="h-4 w-4" /></Link>
                <Link href="/signup" className="inline-flex items-center gap-2 rounded-[1rem] border border-[rgba(26,50,99,0.12)] bg-white px-6 py-3 text-sm font-black">Sign up</Link>
              </div>
            </div>
          </section>
        </main>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-cream)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.8rem] border border-[rgba(26,50,99,0.12)] bg-white p-6 shadow-[0_30px_90px_rgba(26,50,99,0.08)] backdrop-blur lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-2xl border p-4 text-left transition ${active ? 'border-[var(--slot4-dark-bg)] bg-[var(--slot4-dark-bg)] text-white' : 'border-[rgba(26,50,99,0.12)] bg-[var(--slot4-cream)] hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-[2.2rem] border border-[rgba(26,50,99,0.12)] bg-[var(--slot4-cream)] p-5 sm:p-7">
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[var(--slot4-accent)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--slot4-dark-bg)] transition hover:-translate-y-0.5">
